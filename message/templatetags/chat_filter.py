...
def not_seen(context, msg):
    request = context.get('request')
    msg_chat = msg.chat_box.all()
    fchb = msg_chat.get(friend__resipient = request.user)
    print(fchb.unseen_messages.all())
    if fchb.unseen() > 0:
...
# register.filter('not_seen', not_seen)
